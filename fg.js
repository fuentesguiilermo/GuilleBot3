...
const instafg = 'https://www.instagram.com/hg18._'
//--anti delete 
antidel = false
ngetik = false
vn = true
...
const vcard1 = 'BEGIN:VCARD\n' 
 + 'VERSION:3.0\n' 
 + 'FN:𝐺𝑢ͥ𝑖𝑙𝑙𝑒𝑟ͣ𝑚ͫ𝑜 ͢𝑈𝑤𝑈\n'
 + 'ORG:Dueño de Guillermo-Bot;\n' 
...
if (!isVerify) return reply(userB(prefix))
  if (isBanned) return reply(banf())
const fgnum = `50255988363@s.whatsapp.net`
const crisnum = "50255988363@s.whatsapp.net"
...
case 'support':
 if (!isVerify) return reply(userB(prefix))
     Guillermo = `📌 *Grupo soporte del Bot*\n\n${soportefg}`
      reply(guille) 
   break
...
//Fg.sendMessage(from, {displayname: "jeff", vcard: vcard2}, MessageType.contact, { quoted: mek})
                    .then((res) => Fg.sendMessage(from, `El es mi creador`, text, {quoted: res}))
 break
...
if (!isVerify) return reply(userB(prefix))
  if (isBanned) return reply(banf())
  yoi = q
  if (args.length < 1) return reply('✳️ Ingrese las funciones que desee en el Bot *Guillermo*')
...
case 'enlinea':
 if (!isVerify) return reply(userB(prefix))
  ...
  if (!isGroup) return reply(group())
			    ...
			    Fg.sendMessage(from, 'En linea:\n' + online.map(v => '- @' + v.replace(/@.+/, '')).join`\n` + `\n*©POWERED BY Guillermo ┃ ᴮᴼᵀ*`, text, { quoted: mek,
...
var mantap2 = await convertSticker(bas642, `💎 @fg98._`, `🔮 Guillermo┃ ᴮᴼᵀ`)
...
var mantap2r = await convertSticker(bas642r, `💎 @fg98._`, `🔮 Guillermo ┃ ᴮᴼᵀ`)
...
var mantap2c = await convertSticker(bas642c, `💎 @hg18._`, `🔮 Guille ┃ ᴮᴼᵀ`)
...
_📤 Soy demasiado lento, se esta enviando la música_`
...
case 'profile':
					...
					ppimg = 'https://i.ibb.co/PZNv21q/Profile-HG18.jpg'
					}
					 profile = `╭────「 *PERFIL* 」
│• *🔖 Nombre:* ${pushname}
